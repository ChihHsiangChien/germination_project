...
*   **範例**：若 $HR = 1.5$，代表在任何時間點，去芒組發芽的可能性是未去芒組的 1.5 倍。
...
### 4. 風險比 (Hazard Ratio, HR) —— 「勝算」指標
*   這是 Cox 模型產出的核心數值。
*   **解讀範例**：若 `HR = 5.0` 表示 `去芒發芽勝算為未去芒的 5 倍`：
    *   代表在實驗中的每一個瞬間，去芒組種子發芽的可能性都是未去芒組的 5 倍。
    *   這常用來描述處理對發芽「動力」的提升倍數。
